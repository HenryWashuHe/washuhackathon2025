...
export function RiskScoreDisplay({
  ...
  const social = socialRisk !== undefined ? getRiskLevel(socialRisk) : null
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Risk Breakdown */}
      <div className={`grid ${socialRisk !== undefined ? 'grid-cols-3' : 'grid-cols-2'} gap-4`}>
        {/* Environmental Risk */}
        ...
        {socialRisk !== undefined && social && (
          <Card className="p-4 bg-muted/30">
            <div className="flex items-center gap-2 mb-3">
              ...
              <Label className="text-sm font-medium">Social Risk</Label>
            </div>
            
            <div className={`text-3xl font-bold mb-2 ${social.color}`}>
              {socialRisk.toFixed(0)}
            </div>
            
            <div className="h-2 bg-slate-200 dark:bg-slate-700 rounded-full overflow-hidden mb-2">
              <div 
                className={`h-full ${social.bgColor} transition-all duration-500`}
                style={{ width: `${socialRisk}%` }}
              />
            </div>
            
            <p className={`text-xs font-semibold ${social.color}`}>
              {social.label}
            </p>
          </Card>
        )}
